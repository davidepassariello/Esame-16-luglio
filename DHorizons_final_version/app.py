...
@app.route("/login", methods=["GET", "POST"])    #Richiede la pagina .html attraverso la get (altrove è implicito)
def login():
...
@app.route("/logout")
def logout():
    session.pop("admin_logged_in", None)   #pop elimina elemento dalla lista; none per non far crashare il sito se la chiave admin_logged_in non esiste
